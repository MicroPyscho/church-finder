...
export default function ResultsPage() {
  ...
  const mut = useMutation({
    ...
    onSuccess: (data) => {
      setResults(data);
      if ((data.total ?? 0) === 0) (window as any).umami?.track("zero-results", { query });
    },
  });
  ...
  return (
    <div style={{ minHeight: "calc(100svh - 52px)", background: "var(--bg)" }}>
      <div className="wrap" style={{ paddingTop: 28, paddingBottom: 80 }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:28, gap:16, flexWrap:"wrap" }}>
          <div>
            <button onClick={() => navigate("/")}
              style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:"0.78rem", color:"var(--ink3)", background:"none", border:"none", cursor:"pointer", marginBottom:10, padding:0, transition:"color .15s" }}
              onMouseEnter={e => (e.currentTarget.style.color="var(--ink)")}
              onMouseLeave={e => (e.currentTarget.style.color="var(--ink3)")}
            >
              <ArrowLeft size={13} /> New search
            </button>
            <h1 style={{ fontFamily:"'Gabarito'", fontWeight:900, fontSize:"clamp(22px,3.5vw,32px)", letterSpacing:"-0.03em", color:"var(--ink)", margin:0, lineHeight:1.1 }}>
              {mut.isPending
        ...
        {/* Sources trust strip */}
        {!mut.isPending && total > 0 && (
          <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap", marginBottom:24, padding:"10px 16px", background:"var(--surface)", border:"1px solid var(--line)", borderRadius:12 }}>
            ...
            {SOURCES.map(s => (
              <span key={s} style={{ font:"400 11px 'Space Grotesk'", color:"var(--ink3)", background:"var(--surface2)", borderRadius:6, padding:"2px 8px", whiteSpace:"nowrap" }}>{s}</span>
            ))}
          </div>
        )}
          ...
          <>
            {grouped.map(b => (
              <div key={b.score}>
                {hasIntent && b.items.length > 0 && (
                  <div style={{ display:"flex", alignItems:"center", gap:10, padding:"20px 0 10px", fontSize:"0.68rem", fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", color:"var(--ink3)" }}>
                    <span>{b.label}</span>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:1 }}>
                  {b.items.map((p: any) => (
                    <PropertyCard key={p.id} property={p} matchScore={p._score} criteria={p._criteria} />
                  ))}
                </div>
              </div>
            ))}
            ...
            {results?.pages > 1 && (
              <div style={{ display:"flex", alignItems:"center", gap:12, justifyContent:"center", marginTop:40 }}>
                ...
                <button onClick={() => setPage(page + 1)} disabled={page >= results.pages}
