...
export default function SearchPage() {
  ...
  const mut = useMutation({
    mutationFn: (query:string) => api.post("/api/search",{query,filters:{},page:1}).then(r=>r.data),
    onSuccess: (data) => {
  ...
  function search(str=q) {
    if (!str.trim()) return;
    setEdgeCase(null); setShowFollowUp(false);
    mut.mutate(str);
  }
  ...
  function handleVoice() {
    ...
    r.onresult=(e:any)=>{ const t=e.results[0][0].transcript; setQ(t); setTimeout(()=>search(t),200); };
    r.start();
  }
  ...
  return (
    <div className="search-hero">
      ...
      <p className="search-hero__sub">
        Describe exactly what you need. We search 30+ sources simultaneously and show every match — from perfect to partial.
      </p>
      ...
      <div className="searchbar-wrap">
        <div className="searchbar">
          <input
            ...
            placeholder="e.g. affordable church with parking, under £150k, Yorkshire…"
            value={q}
          ...
          <div className="searchbar__actions">
            <button className={`btn-voice${recording?" recording":""}`} onClick={handleVoice} title="Voice search">
              {recording?<MicOff size={15}/>:<Mic size={15}/>}
            </button>
            <button className="btn-search" onClick={()=>search()} disabled={mut.isPending||!q.trim()}>
              {mut.isPending
        ...
        <div className="examples">
          {EXAMPLES.map(ex=>(
            <button key={ex} className="example" onClick={()=>{setQ(ex);inputRef.current?.focus();}}>{ex}</button>
          ))}
        </div>
      </div>

      {edgeCase && <EdgeCase data={edgeCase} onSuggestionClick={s=>{setEdgeCase(null);setQ(s);search(s);}}/>}
